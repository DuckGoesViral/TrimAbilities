...
# Team-agnostic: heal-pool regen for each team's members (iterate the registry).
data modify storage trimabilities:iter q set from storage teams:registry names
data modify storage trimabilities:iter cb set value "trimabilities:spells/heal_apply"
...
# Team-agnostic: necro-summon retaliation per team (iterate the registry; also
# fixes the old always-!AquaTeam proximity check). Necromancer team handled after.
data modify storage trimabilities:iter q set from storage teams:registry names
data modify storage trimabilities:iter cb set value "trimabilities:spells/necro_apply"
function trimabilities:teamiter/run
execute as @e[tag=necroSummon,team=necromancer,scores={necroDamage=5..}] at @s if entity @e[type=!player,type=!item,type=!marker,type=!item_display,type=!armor_stand,team=!necromancer,distance=1..15] run damage @s 0 mob_attack by @e[type=!item,type=!armor_stand,type=!marker,type=!player,distance=..30,limit=1,team=!necromancer]
